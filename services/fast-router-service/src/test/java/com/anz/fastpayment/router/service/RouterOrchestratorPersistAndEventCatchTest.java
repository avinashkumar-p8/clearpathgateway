...
class RouterOrchestratorPersistAndEventCatchTest {

	@Test
	void repositories_throw_and_eventPublisher_throws_are_swallowed() throws Exception {
		...
		when(inboundRepo.save(any())).thenThrow(new RuntimeException("inbound-save-fail"));
		when(inboundRepo.findById("PE")).thenThrow(new RuntimeException("inbound-find-fail"));
		...
		doThrow(new RuntimeException("evt" )).when(evt).publishPaymentReceivedEvent(anyString(), anyString(), anyString());
		...
		doThrow(new RuntimeException("unified-save-fail")).when(unifiedRepo).save(any());
		...
		when(uniProv.getIfAvailable()).thenReturn(unifiedRepo);
